...
<tr style="text-align: left;">
	...
	<option value="<?php echo $AvatarSize1W; ?>" selected="selected"><?php echo $AvatarSize1W; ?></option><?php echo "\n";
        $r = 1;
        while ($r <= 100) { ?><option value="<?php echo $r ?>"><?php echo $r; ?></option><?php echo "\n";
            ++$r;
        } ?>
</select></td>
</tr><tr style="text-align: left;">
...
<option value="<?php echo $AvatarSize1H; ?>" selected="selected"><?php echo $AvatarSize1H; ?></option><?php echo "\n";
        $s = 1;
        while ($s <= 100) { ?><option value="<?php echo $s ?>"><?php echo $s; ?></option><?php echo "\n";
            ++$s;
        } ?>
</select></td>
</tr></table>
...
<div class="TableMessage" style="text-align: center;">Your avatar url is not a valid web url.<br />&#160;</div>
	<?php }
            if (!is_numeric($_POST['AvatarSizeW'])) {
                $_POST['AvatarSizeW'] = 100;
            }
            if ($_POST['AvatarSizeW'] >= 100) {
                $_POST['AvatarSizeW'] = 100;
            }
            if (!is_numeric($_POST['AvatarSizeH'])) {
                $_POST['AvatarSizeH'] = 100;
            }
            if ($_POST['AvatarSizeH'] >= 100) {
                $_POST['AvatarSizeH'] = 100;
            }
...
<?php sql_free_result($result);
    ...
    if ($_POST['update'] == "now") {
        if ($_POST['act'] == "settings" &&
            ...
            if (!is_numeric($_POST['RepliesPerPage'])) {
                $_POST['RepliesPerPage'] = "10";
            }
            if (!is_numeric($_POST['TopicsPerPage'])) {
                $_POST['TopicsPerPage'] = "10";
            }
            if (!is_numeric($_POST['MessagesPerPage'])) {
                $_POST['MessagesPerPage'] = "10";
            }
...
if ($_GET['act'] == "profile") {
    if ($_POST['update'] != "now") {
        ...
        if ($BirthMonth != null && $BirthDay != null && $BirthYear != null) {
            ...
            if (pre_strlen($BirthMonth) == "1") {
                $BirthMonth = "0".$BirthMonth;
            }
            if (pre_strlen($BirthDay) == "1") {
                $BirthDay = "0".$BirthDay;
            }
...
<?php sql_free_result($result);
    ...
    if ($_POST['update'] == "now") {
        if ($_POST['act'] == "profile" &&
            $_SESSION['UserGroup'] != $Settings['GuestGroup']) {
            if (preg_match("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", $_POST['EventDay'])) {
                $_POST['EventDay'] = preg_replace("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", "$2/$3/$1", $_POST['EventDay']);
            }
            $Error = "No";
            if ($_POST['Website'] == "") {
                $_POST['Website'] = "http://";
...
<?php } if (pre_strlen($_POST['OldPass']) > "60") {
    $Error = "Yes"; ?>
