...
/*
    ...
    $FileInfo: system.php - Last Update: 8/26/2024 SVN 1048 - Author: cooldude2k $
*/
...
if ($use_old_session == true) {
    ...
    function sql_session_read($id)
    {
        ...
        if ($sessionExists == 0) {
            ...
            sql_query(sql_pre_query(
                "DELETE FROM \"" . $sqltable . "sessions\" WHERE \"session_id\" <> '%s' AND \"ip_address\" = '%s' AND \"user_agent\" = '%s'",
            ...
            sql_query(sql_pre_query(
                "INSERT INTO \"" . $sqltable . "sessions\" (\"session_id\", \"session_data\", \"serialized_data\", \"user_agent\", \"client_hints\", \"ip_address\", \"expires\") VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %i)",
                array($id, $temp_session_data, $alt_temp_session_data, $temp_user_agent, $client_hints_json, $temp_user_ip, $time)
            ), $SQLStat);
            return '';
        } else {
    ...
    function sql_session_write($id, $data)
    {
        ...
        if ($sessionExists == 0) {
            ...
            sql_query(sql_pre_query(
                "INSERT INTO \"" . $sqltable . "sessions\" (\"session_id\", \"session_data\", \"serialized_data\", \"user_agent\", \"client_hints\", \"ip_address\", \"expires\") VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %i)",
                array($id, $data, serialize($_SESSION), $temp_user_agent, $client_hints_json, $temp_user_ip, $time)
            ), $SQLStat);
        } else {
            // Update existing session data
            sql_query(sql_pre_query(
                "UPDATE \"" . $sqltable . "sessions\" SET \"session_data\" = '%s', \"serialized_data\" = '%s', \"user_agent\" = '%s', \"client_hints\" = '%s', \"ip_address\" = '%s', \"expires\" = %i WHERE \"session_id\" = '%s'",
                array($data, serialize($_SESSION), $temp_user_agent, $client_hints_json, $temp_user_ip, $time, $id)
            ), $SQLStat);
        }
