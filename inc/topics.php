...
<?php }
    ...
    if ($CatPermissionInfo['CanViewCategory'][$ForumCatID] == "yes" &&
        ...
        if ($ForumType != "redirect") {
            if ($NumberViews == 0 || $NumberViews == null) {
                $NewNumberViews = 1;
            }
            if ($NumberViews != 0 && $NumberViews != null) {
                $NewNumberViews = $NumberViews + 1;
            }
        ...
        if ($ForumType == "redirect") {
            if ($RedirectTimes == 0 || $RedirectTimes == null) {
                $NewRedirTime = 1;
            }
            if ($RedirectTimes != 0 && $RedirectTimes != null) {
                $NewRedirTime = $RedirectTimes + 1;
            }
...
<?php }
    while ($i < $num) {
        ...
        if (!is_numeric($TopicStat)) {
            $TopicStat = 1;
        }
        if (!is_numeric($OldForumID)) {
            $OldForumID = $ForumID;
        }
        if ($OldForumID == "0") {
            $OldForumID = $ForumID;
...
<?php } if ($_GET['act'] == "create") {
    ...
    if ($PermissionInfo['CanMakeTopics'][$ForumID] == "no" || $CanHaveTopics == "no") {
        redirect("location", $rbasedir.url_maker($exfile['index'], $Settings['file_ext'], "act=".$viewvar, $Settings['qstr'], $Settings['qsep'], $prexqstr['index'], $exqstr['index'], false));
        ob_clean();
        header("Content-Type: text/plain; charset=".$Settings['charset']);
        $urlstatus = 302;
...
<?php } if ($_GET['act'] == "maketopic" && $_POST['act'] == "maketopics") {
    ...
    if (!isset($_POST['TopicDesc'])) {
        $_POST['TopicDesc'] = null;
    }
    if (!isset($_POST['TopicPost'])) {
        $_POST['TopicPost'] = null;
    }
...
<td class="TableColumn3">
<table style="width: 100%; height: 25%; text-align: center;">
<?php if (pre_strlen($_POST['TopicName']) > "50") {
    $Error = "Yes";  ?>
<tr>
...
<?php } }*/ if (pre_strlen($_POST['TopicDesc']) > "80") {
    $Error = "Yes";  ?>
<tr>
	<td><span class="TableMessage">
...
<?php } if ($PermissionInfo['CanMakeTopics'][$ForumID] == "no" || $CanHaveTopics == "no") {
    $Error = "Yes"; ?>
<tr>
	<td><span class="TableMessage">
