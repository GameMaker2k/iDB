    ...
    if ($_GET['act'] == "read") {
        ...
        while ($is < $num) {
            ...
            if ($User1CanUseBBTags == "yes") {
                $MessageText = bbcode_parser($MessageText);
            }
            ...
            if ($User1CanExecPHP == "yes") {
                $MessageText = php_execute($MessageText);
            }
            ...
            if ($User1CanDoHTML == "yes") {
                $MessageText = do_html_bbcode($MessageText);
            }
...
<?php }
    ...
    if (is_numeric($_POST['post'])) {
        $_POST['post'] = intval($_POST['post'], 10);
    }
    if (!isset($_POST['post']) or !is_numeric($_POST['post']) or $_POST['post'] == null) {
        $_POST['post'] = 0;
    }
    if ($_POST['post'] > 0) {
...
(%i, %i, %i, '%s', '%s', '%s', '%s', %i, %i, '%s')", array($_POST['post'],$_SESSION['UserID'],$SendMessageToID,$_SESSION['MemberName'],$_POST['MessageName'],$_POST['Message'],$_POST['MessageDesc'],$LastActive,0,$User1IP));
    ...
    if ($_POST['post'] > 0) {
        $msglinkback = "Click <a href=\"".url_maker($exfile['messenger'], $Settings['file_ext'], "act=read&id=".$_POST['post'], $Settings['qstr'], $Settings['qsep'], $prexqstr['messenger'], $exqstr['messenger'])."\">here</a> to go back to message.";
    }
    ?><tr>
