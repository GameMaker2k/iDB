...
while ($is < $num) {
    ...
    if ($User1CanUseBBTags == "yes") {
        $EventText = bbcode_parser($EventText);
    }
    ...
    if ($User1CanExecPHP == "yes") {
        $EventText = php_execute($EventText);
    }
    ...
    if ($User1CanDoHTML == "yes") {
        $EventText = do_html_bbcode($EventText);
    }
...
<?php }  if ($_GET['act'] == "makeevent" && $_POST['act'] == "makeevents") {
    if (preg_match("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", $_POST['EventStart'])) {
        $_POST['EventStart'] = preg_replace("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", "$2/$3/$1", $_POST['EventStart']);
    }
    if (preg_match("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", $_POST['EventEnd'])) {
        $_POST['EventEnd'] = preg_replace("/([0-9]{4})\-([0-9]{2})\-([0-9]{2})/", "$2/$3/$1", $_POST['EventEnd']);
    }
    if ($GroupInfo['CanAddEvents'] == "no") {
        redirect("location", $rbasedir.url_maker($exfile['index'], $Settings['file_ext'], "act=view", $Settings['qstr'], $Settings['qsep'], $prexqstr['index'], $exqstr['index'], false));
        ob_clean();
