...
if ($_GET['act'] == "list" || $_GET['act'] == "getactive") {
    ...
    if ($_GET['orderby'] != null) {
        ...
        if ($_GET['orderby'] == "post") {
            $orderlist = "order by \"PostCount\"";
        }
        if ($_GET['orderby'] == "posts") {
            $orderlist = "order by \"PostCount\"";
        }
        if ($_GET['orderby'] == "karma") {
            $orderlist = "order by \"Karma\"";
        }
    ...
    switch ($_GET['list']) {
        case "members":
            $NumberMembers = sql_count_rows(
                sql_pre_query(
                    "SELECT COUNT(*) AS cnt FROM \"".$Settings['sqltable']."sessions\" WHERE \"expires\" >= %i AND \"serialized_data\" NOT LIKE '%s'",
        ...
        case "guests":
            $NumberMembers = sql_count_rows(
                sql_pre_query(
                    "SELECT COUNT(*) AS cnt FROM \"".$Settings['sqltable']."sessions\" WHERE \"expires\" >= %i AND \"serialized_data\" LIKE '%s'",
                    array($uoltime, "%UserGroup|s:".strlen($Settings['GuestGroup']).":\"".$Settings['GuestGroup']."\";%")
                ),
                $SQLStat
            );
        ...
        default:
            $NumberMembers = sql_count_rows(
                sql_pre_query(
                    "SELECT COUNT(*) AS cnt FROM \"".$Settings['sqltable']."sessions\" WHERE \"expires\" >= %i",
                    array($uoltime)
                ),
                $SQLStat
            );
...
while ($i < $num) {
    ...
    if (isset($UserSessInfo)) {
        $UserSessInfo = null;
    }
    //$UserSessInfo = unserialize_session($session_data);
    $UserSessInfo = unserialize($serialized_data);
    if (!isset($UserSessInfo['ShowActHidden'])) {
        $UserSessInfo['ShowActHidden'] = "no";
    }
    ...
    if ($PreFileName == $exfile['topic'].$Settings['file_ext']) {
        if (isset($ChkID['id'])) {
            $prenum = sql_count_rows(sql_pre_query("SELECT COUNT(*) AS cnt FROM \"".$Settings['sqltable']."topics\" WHERE \"id\"=%i LIMIT 1", array($ChkID['id'])), $SQLStat);
            $prequery = sql_pre_query("SELECT * FROM \"".$Settings['sqltable']."topics\" WHERE \"id\"=%i LIMIT 1", array($ChkID['id']));
...
<td class="TableColumn3" style="text-align: center;"><a<?php if ($GroupInfo['HasAdminCP'] == "yes") { ?> title="<?php echo htmlentities($session_user_agent, ENT_QUOTES, $Settings['charset']); ?>"<?php } ?> href="<?php echo url_maker($exfile['member'], $Settings['file_ext'], "act=view&id=".$UserSessInfo['UserID'], $Settings['qstr'], $Settings['qsep'], $prexqstr['member'], $exqstr['member']); ?>"><?php if ($AmIHiddenUser == "yes") {
    echo "*";
} echo $UserSessInfo['MemberName']; ?></a>
...
if ($_GET['act'] == "view") {
    ...
    if (!is_numeric($ViewMem['LevelID'])) {
        $ViewMem['LevelID'] = null;
    }
    $ViewMem['RankID'] = $result_array['RankID'];
    if (!is_numeric($ViewMem['RankID'])) {
        $ViewMem['RankID'] = null;
    }
    ...
    if ($ViewMem['LevelID'] !== null && $ViewMem['LevelID'] != 0) {
        ...
        if ($lresult !== false && $lnum > 0) {
            ...
            $ViewMem['Level'] = $lresult_array['Name'];
        } else {
            $ViewMem['Level'] = "";
        }
    ...
    if ($ViewMem['RankID'] !== null && $ViewMem['RankID'] != 0) {
        ...
        if ($rresult !== false && $rnum > 0) {
            ...
            $ViewMem['Rank'] = $rresult_array['Name'];
        } else {
            $ViewMem['Rank'] = "";
        }
...
<?php }
        ...
        if ($_SESSION['UserID'] == 0 || $_SESSION['UserID'] == null) {
            ...
            if ($_POST['anonymouslogin'] == "true") {
                $_SESSION['AnonymousLogin'] = "yes";
                $_COOKIE['AnonymousLogin'] = "yes";
            } else {
                $_SESSION['AnonymousLogin'] = "no";
                $_COOKIE['AnonymousLogin'] = "no";
            }
            $_SESSION['PreViewingTitle'] = "Act: ";
            $_SESSION['ViewingTitle'] = "Logging in";
            $_SESSION['ExtraData'] = "currentact:".$_GET['act']."; currentcategoryid:0; currentforumid:0; currenttopicid:0; currentmessageid:0; currenteventid:0; currentmemberid:0;";
...
if (!isset($_POST['loginemail'])) {
    $_POST['loginemail'] = "false";
}
            if (pre_strlen($_POST['userpass']) > "60") {
                $Error = "Yes";  ?>
<tr>
...
<?php } if (pre_strlen($_POST['username']) > "30") {
    $Error = "Yes";  ?>
<tr>
	<td><span class="TableMessage">
...
<?php } if ($_POST['loginemail'] == "true" && filter_var($_POST['loginemail'], FILTER_VALIDATE_EMAIL)) {
    $Error = "Yes";  ?>
<tr>
	<td><span class="TableMessage">
...
<?php }
            ...
            if ($Error != "Yes") {
                ...
                if ($numlog >= 1) {
                    ...
                    if ($HashType == "GuestPassword") {
                        $YourPassword = null;
                        $UpdateHash = false;
                    }
                    //iDB hashing system
                    if ($HashType == "ODFH") {
                        $YourPassword = PassHash2x($_POST['userpass']);
                    }
                    if ($HashType == "IPB2") {
                        $YourPassword = hash2xkey($_POST['userpass'], $HashSalt);
                    }
                    if ($HashType == "DF4H") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha1");
                    }
                    if ($HashType == "iDBH2") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "md2");
                    }
                    if ($HashType == "iDBH4") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "md4");
                    }
                    if ($HashType == "iDBH5") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "md5");
                    }
                    if ($HashType == "iDBH") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha1");
                    }
                    ...
                    if ($HashType == "iDBH224") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha224");
                    }
                    if ($HashType == "iDBH256") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha256");
                    }
                    if ($HashType == "iDBH384") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha384");
                    }
                    if ($HashType == "iDBH512") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha512");
                    }
                    if ($HashType == "iDBH3224") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha3-224");
                    }
                    if ($HashType == "iDBH3256") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha3-256");
                    }
                    if ($HashType == "iDBH3384") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha3-384");
                    }
                    if ($HashType == "iDBH3512") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "sha3-512");
                    }
                    if ($HashType == "iDBHRMD128") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "ripemd128");
                    }
                    if ($HashType == "iDBHRMD160") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "ripemd160");
                    }
                    if ($HashType == "iDBHRMD256") {
                        $YourPassword = b64e_hmac($_POST['userpass'], $JoinedPass, $HashSalt, "ripemd256");
                    }
                    if ($HashType == "iDBHRMD320") {
                    ...
                    if (hash_equals($YourPassTry, $YourPassword) == false) {
                        $passright = false;
                    }
                    if (hash_equals($YourPassTry, $YourPassword) == true) {
                        $passright = true;
...
<?php } if (pre_strlen($_POST['Password']) > "60") {
    $Error = "Yes"; ?>
<tr>
	<td><span class="TableMessage">
	<br />Your password is too big.<br />
...
<?php } if ($_POST['Website'] == "") {
    $_POST['Website'] = "http://";
                        ...
                        if (!filter_var($_POST['Website'], FILTER_VALIDATE_URL) && $_POST['Website'] != "http://" && $_POST['Website'] != "https://") {
...
<?php } if ($_POST['Avatar'] == "") {
    $_POST['Avatar'] = "http://";
                        ...
                        if (!filter_var($_POST['Avatar'], FILTER_VALIDATE_URL) && $_POST['Avatar'] != "http://" && $_POST['Avatar'] != "https://") {
...
<?php } if ($Error != "Yes") {
    ...
    $_POST['Joined'] = $utccurtime->getTimestamp();
    $_POST['LastActive'] = $utccurtime->getTimestamp();
    $_POST['Signature'] = "";
    $_POST['Interests'] = "";
    $_POST['Title'] = "";
    $_POST['PostCount'] = "0";
    ...
    if ($Settings['AdminValidate'] == "on" || $Settings['AdminValidate'] != "off") {
        $ValidateStats = "no";
        $yourgroupname = $Settings['ValidateGroup'];
    }
    if ($Settings['AdminValidate'] == "off" || $Settings['AdminValidate'] != "on") {
        $ValidateStats = "yes";
        $yourgroupname = $Settings['MemberGroup'];
    }
